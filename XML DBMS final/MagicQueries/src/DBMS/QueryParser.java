...
public class QueryParser {
	...
	public void insert() throws Exception 															// Insert a row below table		
	{
		...
		MagicQueriesLogger.logger.info("Query executed Successfully , insertion done in table:"+tableName);
	}
	...
	{															
		...
		if (input.length == 1) 
		{
			...
			MagicQueriesLogger.logger.info("Query executed Successfully  delete All done table:"+tableName);
		} else 
		{
			condition = getCondition(input[1]);
			controller.delete(condition); 											//to delete a specific row
			MagicQueriesLogger.logger.info("Query executed Successfully  deletion done table:"+tableName);
		}
	}
	...
	public void update() throws Exception 															// update a value
	{
		...
		if (input.length==2)
		{
			...
			MagicQueriesLogger.logger.info("Query executed Successfully  updateAll done in table:"+tableName);

		}else
		{
			condition=getCondition(input[2]);												// update specific rows with the new values
			controller.update(columns,values,condition);
			MagicQueriesLogger.logger.info("Query executed Successfully  update cells done in table:"+tableName);
		}
	}
	...
	public void select() throws Exception															// select table or a specific row 
	{
		...
		if (input[0].length()==8 && input.length==2)										// to select all the table
		{
			...
			MagicQueriesLogger.logger.info("Query executed Successfully  select All Table done in table:"+tableName);
		}else if (input[0].length()==8 && input.length==3)									// to select some rows
		{
			...
			MagicQueriesLogger.logger.info("Query executed Successfully  select rows done in table:"+tableName);
		}else if (input .length==2)															// to select all the column chosen before
		{
			...
			MagicQueriesLogger.logger.info("Query executed Successfully  select the choosen columns done in table:"+tableName);
		}else																				// to select some cells 
		{
			columns=getArray(input[0].substring(6));										
			condition=getCondition(input[2]);
			table=controller.selectColumns(columns,condition);
			MagicQueriesLogger.logger.info("Query executed Successfully  selection done in table:"+tableName);
		}	
	...
	public void create() throws Exception									// Create new table
	{
		...
		for (int i=0;i<temp.length;i++)
		{
			...
			MagicQueriesLogger.logger.info("Creating column with Name : "+columns[i] +" Attribute :" +dataTypes[i]);
		}
		MagicQueriesLogger.logger.info("Query executed Successfully   creation of new in table :"+tableName);
	}
	...
	public void drop() throws Exception														//  Drop exist table
	{
		...
		MagicQueriesLogger.logger.info("Query executed Successfully  droping done in table:"+tableName);
	}
